...
function AdminWarpPanel:initialise()
    ...
    local listY, listHeight = 60, self.height - 150
    ...
    self.deleteBtn = ISButton:new(margin + btnWidth + 5, btnY, btnWidth, btnHeight, "Delete", self, AdminWarpPanel.onDeletePortal)
    self.deleteBtn.backgroundColor = {r=0.6, g=0.2, b=0.2, a=1}
    self:addChild(self.deleteBtn)
    self.toggleBtn = ISButton:new(margin + (btnWidth + 5) * 2, btnY, btnWidth, btnHeight, "Toggle", self, AdminWarpPanel.onTogglePortal)
    self.toggleBtn.backgroundColor = {r=0.2, g=0.4, b=0.6, a=1}
    self:addChild(self.toggleBtn)
    self.teleportBtn = ISButton:new(margin + (btnWidth + 5) * 3, btnY, btnWidth + 20, btnHeight, "Teleport", self, AdminWarpPanel.onTeleportPortal)
...
function AdminWarpPanel:refreshListDisplay()
    ...
    for i, portal in ipairs(self.portals) do
        ...
        local displayText = string.format("%-25s %-20s %-12s %-15s",
            ...
            portal.active and "Active" or "Inactive",
            factionDisplay
        )
...
function AdminWarpPanel:sendUpdateToServer()
    if isClient() then
        print("Sending " .. #self.portals .. " portals to server")
